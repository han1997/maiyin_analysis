...
- 入住时间重合、同日多次非重合、选定时间范围及滚动 7/30/365 天高频预警和评分。
...
- 入住重叠：同房间也产生预警，不同旅馆或房号额外加分，`min(35, 20 + 重叠对数 × 2 + 不同地点对数 × 5)`。
- 同日非重叠入住不少于 4 次：`min(45, 25 + (次数 - 4) × 5)`。
- 频次超过阈值：`min(80, 45 + (次数 - 阈值) × 6)`。
- 设置任一入住时间边界时，只计算范围内频次；未设置边界时，计算滚动 7/30/365 天频次，两类频次不会同时计分。
- 总分最高 100；高风险、中风险、关注的起始分分别为 80、55、30。
